...
# Can get another model with experimental values and direct training model through flexible comments
# Pre-trained boiling point prediction model:
import pickle
# Load pre-trained model and Scaler
pretrained_model = load_model('best_model_Tb.h5')
...
best_model.save('best_model_Tc.h5')  # Save model structure and weights
with open('best_scaler_Tc.pkl', 'wb') as f:
    pickle.dump(best_scaler, f)  # Save scaler

# Save model configuration
model_config = best_model.get_config()
with open('model_config_Tc.json', 'w') as f:
