...
def solve_cubic_equation(a, b, R, T, p_param):
    ...
    c2 = b - (R * T) / p_param
    c2_div3 = c2 / 3.0  # Pre-compute frequently used value
    
    # Use more efficient cubic calculation (multiplication instead of pow)
    c1 = a / p_param - 3 * tf.square(b) - 2 * b * R * T / p_param
    ...
    p = c1 - 3.0 * tf.square(c2_div3)  # Eliminate large number subtraction
    q_c2_div3_cubed = c2_div3 * c2_div3 * c2_div3  # Optimize cubic calculation
    q = 2.0 * q_c2_div3_cubed - c1 * c2_div3 + c0

    # Enhanced numerical stability calculation
    sqrt_input = tf.maximum(-p / 3.0, 1e-12)  # Ensure non-negative input
    ...
    denominator = 2.0 * sqrt_neg_p_cubed + 1e-12
    
    # Limit the range of the inverse trigonometric function input
    argument = tf.clip_by_value(-q / denominator, -1.0+1e-8, 1.0-1e-8)  # Stricter boundary protection
    theta = tf.acos(argument)

    # Vectorized angle calculation
    k_vals = tf.constant([0.0, 1.0, 2.0], dtype=tf.float32)
    angles = (tf.expand_dims(theta, -1) + 2 * np.pi * k_vals) / 3.0
    cos_terms = tf.cos(angles)
    
    # Final root calculation (keep broadcast mode)
    roots = 2.0 * tf.expand_dims(sqrt_neg_p, -1) * cos_terms - tf.expand_dims(c2_div3, -1)
...
def process_roots_batch(roots, b_batch):
    ...
    safety_ratio = 1.001
    valid_b = tf.maximum(b_batch, 1e-6)  # Ensure b is not zero or negative
    ...
    real_roots_mask = tf.abs(roots_imag) < 1e-6
    ...
    # Valid case handling
    masked_roots = tf.where(valid_roots_mask, roots_real, tf.float32.max)
    ...
    V_sv_invalid = valid_b * 1000.0  # Sufficiently large but still maintains physical meaning
...
def calc_H_tf(input_tensor, factor):
    Tb = input_tensor[:, 0]
    Tc = input_tensor[:, 1]
    pc = input_tensor[:, 2] / 10.0  # bar to MPa

    R = 8.31451
    a0 = 0.457235 * (R * Tc)**2 / pc
    b = 0.077796 * R * Tc / pc
    k = 0.378893 + 1.4897153 * factor - 0.17131848 * tf.square(factor)+0.0196554*tf.pow(factor,3)

    term_inside = 1 + k * (1 - tf.sqrt(Tb / Tc))
    term_inside = tf.maximum(term_inside, 1e-12)  # Prevent negative numbers
    ...
    p_s_initial = pc * 10.0 ** (7.0 * (1 + factor) / 3.0 * (1 - Tc / Tb))
    p_s_initial = tf.maximum(p_s_initial, 1e-6)  # Ensure initial value is not negative
    ...
    def body(p_s, converged, iteration):
        ...
        sqrt2 = tf.sqrt(2.0)
        log_term_sv = tf.math.log(tf.maximum((p_s * (V_sv - b)) / (R * Tb),1e-12))
        log_ratio_sv = tf.math.log(tf.maximum((V_sv + (sqrt2 + 1) * b) / (V_sv + (1 - sqrt2) * b),1e-12))
        ln_phi_sv = Z_sv - 1 - log_term_sv - a / (2**1.5 * b * R * Tb) * log_ratio_sv

        log_term_sl = tf.math.log(tf.maximum((p_s * (V_sl - b)) / (R * Tb),1e-12))
        log_ratio_sl = tf.math.log(tf.maximum((V_sl + (sqrt2 + 1) * b) / (V_sl + (1 - sqrt2) * b),1e-12))
        ln_phi_sl = Z_sl - 1 - log_term_sl - a / (2**1.5 * b * R * Tb) * log_ratio_sl

        ln_phi_diff = ln_phi_sv - ln_phi_sl
        abs_diff = tf.abs(ln_phi_diff)
        newly_converged = abs_diff < 1e-6
        new_converged = tf.logical_or(converged, newly_converged)

        Z_diff = Z_sv - Z_sl + 1e-12
        C = 1 - (ln_phi_diff) / Z_diff
        C_clipped = tf.clip_by_value(C, 0.5, 1.5)
        active_mask = tf.logical_not(new_converged)
        new_p_s = tf.where(active_mask, p_s * C_clipped, p_s)
        new_p_s = tf.maximum(new_p_s, 1e-6)  # Prevent p_s from becoming negative or zero
    ...
    sqrt2 = tf.sqrt(2.0)
    log_term_liq = tf.math.log(tf.maximum((V_sl_final + (sqrt2 + 1) * b) / (V_sl_final + (1 - sqrt2) * b),1e-12))
    H_res_liq = R * Tb * (Z_liq - 1) - (a - Tb * da_dT) / (2 * sqrt2 * b) * log_term_liq

    log_term_vap = tf.math.log(tf.maximum((V_sv_final + (sqrt2 + 1) * b) / (V_sv_final + (1 - sqrt2) * b),1e-12))
    H_res_vap = R * Tb * (Z_vap - 1) - (a - Tb * da_dT) / (2 * sqrt2 * b) * log_term_vap
...
for fold, (train_index, test_index) in enumerate(kf.split(X)):
    ...
    factor_model = Sequential([
        Dense(128, input_dim=X.shape[1]-3, 
            ...
            kernel_initializer='he_normal'),  # Keep intermediate layer normal initialization
        ...
        Dense(1, 
            ...
            activity_regularizer=tf.keras.regularizers.l2(0.5)),  # Add output regularization
        
        # Add constraints to ensure output is in the range [-1, 1]
        tf.keras.layers.Lambda(lambda x: tf.clip_by_value(x, -1, 1))
    ])
