...
def build_critical_model(input_shape):
    # Input layer
    inputs = Input(shape=input_shape)
    ...
    # Output layer: 1 neuron, linear activation
    critical_temp = Dense(1, activation='linear', name='critical_temp')(x)

    return Model(inputs, critical_temp, name='critical_model')
...
def build_joint_model(boiling_model, critical_model, X):
    ...
    # Critical temperature model uses first n-1 features (excluding last feature)
    med_input = tf.keras.layers.Lambda(lambda x: x[:, :-1])(input_layer)  # Take first n-1 columns
    critical_input = tf.keras.layers.concatenate([med_input, boiling_output])
    critical_output = critical_model(critical_input)
...
critical_model = build_critical_model(input_shape=X.shape[1])
...
# Visualize predictions and actuals for entire dataset
plt.figure(figsize=(8, 6))
...
plt.plot([10, 80], [10, 80], 'r-', linewidth=3, label="Ideal Prediction Line")

# Add labels, title and grid
plt.xlabel('Actual pc (bar)', fontsize=12)
plt.ylabel('Predicted pc (bar)', fontsize=12)
plt.title('Compartment Model Prediction vs Actual pc (Entire Dataset)', fontsize=14)
plt.grid(True, which='both', linestyle='--', linewidth=0.5)

# Set axis tick marks
plt.xticks(np.arange(final_actuals.min()-5, final_actuals.max() + 5, step=20), fontsize=10)
plt.yticks(np.arange(final_predictions.min()-5, final_predictions.max() + 5, step=20), fontsize=10)

# Add legend and save image
plt.legend(loc='upper left')
plt.savefig('Actual_vs_Predicted_pc.png', dpi=300, bbox_inches='tight')
plt.show()
