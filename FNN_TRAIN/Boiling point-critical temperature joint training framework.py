...
def build_joint_model(boiling_model, critical_model, X):
    ...
    # Critical temperature model needs boiling model output as additional input
    merged_input = tf.keras.layers.concatenate([input_layer, boiling_output])
    critical_output = critical_model(merged_input)
...
critical_model = build_critical_model(input_shape=X.shape[1]+1)
